...
<template>
  <section class="pt-20 bg-gradient-to-r from-white to-blue-50 dark:from-gray-900 dark:to-gray-800 min-h-screen transition-colors duration-500">
    <div class="container mx-auto px-4 py-16 text-center">
      ...
      <button @click="toggleThemeWithOverlay($event)" class="inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-gray-100 font-semibold rounded-lg transition-colors duration-300">
    ...
    <div v-if="posts.length" class="container mx-auto px-4 py-12">
      ...
      <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        <NuxtLink v-for="post in posts" :key="post.slug" :to="`/blog/${post.slug}`" class="group block rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 bg-white dark:bg-gray-800">
          <div class="h-48 overflow-hidden">
            <img :src="'https://source.unsplash.com/collection/888146/400x300?sig=' + post.slug" alt="" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"/>
          </div>
          <div class="p-6 flex flex-col">
            ...
            <div class="mt-4 text-blue-600 dark:text-blue-400 font-semibold flex items-center group-hover:translate-x-1 transition-transform duration-300">
              Read more
